...
public class ExtentReportGenerateService implements IReporter {
     ...
     * 报告文件存储路径
     */
    private static final String OUTPUT_FOLDER = System.getProperty("user.dir") + File.separator + "target" + File.separator + "test-output";
...
//        log.info("开始产生 extentreports 测试报告");
    ...
    private void init() {
        ...
        String OUTPUT_FOLDER_DIR = OUTPUT_FOLDER + File.separator + "extentreport" + File.separator;
        File reportDirs = new File(OUTPUT_FOLDER_DIR);
        if (!reportDirs.exists() && !reportDirs.isDirectory()) {
            reportDirs.mkdir();
        }
        // 时间格式
        ...
        ExtentHtmlReporter htmlReporter = new ExtentHtmlReporter(OUTPUT_FOLDER_DIR + fullFileName);
        // 设置静态文件的 DNS
        // 怎么样解决 cdn.rawgit.com 访问不了的情况
        htmlReporter.config().setResourceCDN(ResourceCDN.EXTENTREPORTS);

        htmlReporter.config().setDocumentTitle("Hupofire接口自动化测试报告");
        htmlReporter.config().setReportName("Hupofire接口自动化测试报告");
